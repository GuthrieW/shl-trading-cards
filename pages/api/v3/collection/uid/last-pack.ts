...
type CardsQueryResult = 
  | QuantityResult[]
  | { error: unknown }; // Adjust based on your actual error type
...
const index = async (
  ...
  if (method === GET) {
    ...
    if ('error' in cardsResult) {
      response.status(StatusCodes.INTERNAL_SERVER_ERROR).json({ error: 'Failed to retrieve cards' });
      return;
    }
    ...
    if (cardIDs.length > 0) {
      ...
      if ('error' in quantitiesResult) {
        response.status(StatusCodes.INTERNAL_SERVER_ERROR).json({ error: 'Failed to retrieve quantities' });
        return;
      }
    }
    ...
    const combinedResult = cardsResult.map(card => {
      const quantityEntry = (quantitiesResult as QuantityResult[]).find(q => q.cardID === card.cardID);
      return {
        ...card,
        quantity: quantityEntry ? quantityEntry.quantity : 0 // Default to 0 if no quantity found
      };
    });
