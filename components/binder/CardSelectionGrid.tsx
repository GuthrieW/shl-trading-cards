...
const CardSelectionGrid: React.FC<CardSelectionGridProps> = ({
  handleCardSelect,
  displayCards,
  selectedPosition,
  bid,
  ...
  const filteredUserCards =
    selectedUserCards?.rows?.filter((tradeCard) => {
      return !displayCards.some(
        (binderCard) => binderCard?.ownedCardID === tradeCard.ownedCardID
      )
    }) || []

  const transformedCards: binderCards[] = filteredUserCards.map(
    (tradeCard) => ({
      binderID: Number(bid),
      ownedCardID: tradeCard.ownedCardID,
      position: selectedPosition,
      cardID: tradeCard.cardID,
      player_name: tradeCard.player_name,
      playerID: tradeCard.playerID,
      teamID: tradeCard.teamID,
      card_rarity: tradeCard.card_rarity,
      image_url: tradeCard.image_url,
      overall: tradeCard.overall,
      season: tradeCard.season,
    })
  )
        ...
        {selectedUserCardsIsLoading
          ...
          : transformedCards?.map((card, index) => (
              <div
                ...
                className="m-4 relative transition ease-linear shadow-none hover:scale-105 hover:shadow-xl cursor-pointer"
